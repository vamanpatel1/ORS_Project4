...
public class UserModel {
	...
	public List search(UserBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		System.out.println("Umodel : search 1=1");
		if (bean != null) {
			  if (bean.getFirstName() != null && bean.getFirstName().length() > 0) {
			  sql.append(" AND FIRST_NAME like '" + bean.getFirstName() + "%'");
			  System.out.println("Umodel search firstname"); }
			...
			if (bean.getDob() != null && bean.getDob().getTime() > 0) {// bean.getDob().getDate() > 0
				Date date = new Date(bean.getDob().getTime());
				sql.append(" AND DOB = '" + date + "%'");
			}
		...
		if (pageSize > 0) {
			...
			System.out.println("Umodel : limit " + pageNo + "," + pageSize);
		}
